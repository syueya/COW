...
COMMANDS = {
    ...
    "音色": {
        "alias": ["音色", "语音音色"],
        "desc": "查看和设置语音音色, 可选的有 male-botong(思远), Podcast_girl(心悦), boyan-new-hailuo(子轩), female-shaonv(灵儿), YaeMiko_hailuo(语嫣), xiaoyi_mix_hailuo(少泽), xiaomo_sft(芷溪), cove_test2_hailuo(浩翔英文), scarlett_hailuo(雅涵英文), Leishen2_hailuo(雷电将军), Zhongli_hailuo(钟离), Paimeng_hailuo(派蒙), keli_hailuo(可莉), Hutao_hailuo(胡桃), Xionger_hailuo(熊二), Haimian_hailuo(海绵宝宝), Robot-hunter-hailuo(模仿变形金刚), Linzhiling_hailuo(林志林), huafei_hailuo(华妃), lingfeng_hailuo(东北er), male_dongbei-hailuo(老铁), Beijing_hailuo(北京er), JayChou_hailuo(JayJay), Daniel_hailuo(潇然), Bingjiao_zongcai_hailuo(沉韵), female-yaoyao-hd(瑶瑶), murong_sft(晨曦), shangshen_sft(沐珊), kongchen_sft(祁辰), shenteng2_hailuo(沈腾), Guodegang_hailuo(郭德刚), yueyue_hailuo(岳云鹏),"
    },
    "set_openai_api_key": {
